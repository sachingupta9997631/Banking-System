...
def update_acc():
    ...
    def sear_det():
        if str(ent_id.get())=="" and str(ent_pin.get())=="":
            ...
            cur.execute("select * from bank where id=? and pin=?",(str(ent_id.get()),str(ent_pin.get())))  
            a=cur.fetchall()
            if a==[]:
                stat.set(str(""))
                ...
                stat.set(str(""))
                stat.set(str("Account Found..."))
                cur.execute("select name,father,mother,address,phone,pin from bank where id=? and pin=?",(str(ent_id.get()),str(ent_pin.get())))
                d=cur.fetchone()
                name.set(str(d[0]))
                fname.set(str(d[1]))
                mname.set(str(d[2]))
    ...
    def upd_details():
        ...
        cur.execute("UPDATE bank set name=?,father=?,mother=?,address=?,phone=?,pin=? where id=? and pin=?",(a,b,c,d,e,f,str(ent_id.get()),str(ent_pin.get())))
        stat.set(str("The Account Details were Updated Successfully!"))
        db.commit()
        del a,b,c,d,e,f
...
def delete_acc():
    ...
    def del_rec():
        cur.execute("DELETE from bank where id=? and pin=?",(str(ent_id.get()),str(ent_pin.get())))
        stat.set(str("The Account was deleted Successfully!"))
        text.delete(1.0,'end')
        db.commit()
    ...
    def sear_det():
        if str(ent_id.get())=="" and str(ent_pin.get())=="":
            ...
            if a==[]:
                ...
                stat.set(str("Account Found..."))
                text.delete(1.0,'end')
                text.insert(INSERT,str("The Details are: "))
                a=str(ent_id.get())
                b=str(ent_pin.get())
                cur.execute("select id,name,father,mother,address,phone,amount from bank where id=? and pin=?",(a,b))
                c=cur.fetchone()
                text.insert(INSERT,str("\n\n       Account ID : " + str(c[0])))
                text.insert(INSERT,str("\n    Holder Name : " + str(c[1])))
                text.insert(INSERT,str("\n  Father's Name : " + str(c[2])))
                text.insert(INSERT,str("\n Mother's Name : " + str(c[3])))
                text.insert(INSERT,str("\n           Address : " + str(c[4])))
                text.insert(INSERT,str("\nContact Number : " + str(c[5])))
                text.insert(INSERT,str("\n Current Amount : " + str(c[6])))
...
def draw_amt():
    ...
    def upd_det():
        if str(ent_draw.get())=="" and str(ent_id.get())=="" and str(ent_pin.get())=="":
            stat.set(str("No Data in the Fields were Found"))
        else:
            cur.execute("select * from bank where id=?",(str(ent_id.get())))  
            a=cur.fetchall()
            if a==[]:
                ...
                cur.execute("select amount from bank where id=? and pin=?",(str(ent_id.get()),str(ent_pin.get())))
                b=cur.fetchone()
                if b[0]<int(ent_draw.get()):
                    stat.set(str("Amount Insufficient to Withdraw... try Again with Less Amount"))
                else:
                    cur.execute("UPDATE bank set amount=amount-(?) where id=? and pin=?",(str(ent_draw.get()),str(ent_id.get()),str(ent_pin.get())))
                    db.commit()
                    stat.set(str("The Amount was Successfully Updated"))
...
def new_acc():
    ...
    def save_data():
        ...
        if cur.fetchone()!=None:
            stat.set(str("The ID has Already been Taken... Try With Another ID"))
        else:
            cur.execute("insert into bank values(?,?,?,?,?,?,?,?)",(a,b,c,d,e,f,g,h))
            db.commit()
